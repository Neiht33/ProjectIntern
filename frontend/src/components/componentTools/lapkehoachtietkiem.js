...
export default function Lapkehoachtietkiem() {
    ...
    function calculateTotalAmountyear(principal, Initialinvestment, years, interestRate) {
        ...
        let totalAmountYear = totalAmount / 12
    ...
    return (
        <form id='paragraph' onSubmit={getFormData}>
            ...
            <div className='Giatrihienthi'>
                <p>Để đạt được <span id="year-result" class="highlight">{formData.p}  </span> (VNĐ) sau <span id="year-result" class="highlight">{formData.n}  </span> năm với vốn đầu tư ban đầu <span id="year-result" class="highlight">{formData.M}  </span> (VNĐ) và mức lãi suất kỳ vọng <span id="year-result" class="highlight">{formData.i}  </span> (%/năm) thì bạn cần tiết kiệm <span id="money-result" class="highlight">{formattedAmount}</span> (VNĐ) mỗi tháng</p>
            </div>
        </form>
    )
}
